...
import { LAYER_GROUP_LABELS, MAP_LAYERS, type MapLayerConfig } from '@/config/mapLayers';
...
function groupLayers(layers: MapLayerConfig[]): Record<MapLayerGroup, MapLayerConfig[]> {
  ...
  return (
    ...
    >
      ...
      >
        ...
        <span className="text-xs font-medium text-charcoal-muted">{expanded ? 'Hide' : 'Show'}</span>
      </button>
      ...
      {expanded && (
        <div className="max-h-[min(50vh,420px)] space-y-4 overflow-y-auto border-t border-sand px-4 py-3">
            ...
            <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-sand bg-mist/20 p-3 transition-colors hover:bg-mist/35">
              ...
              <span>
                <span className="block text-sm font-semibold text-charcoal">
                  ...
                  {detectionCount != null && (
                    <span className="ml-1.5 font-mono text-xs text-sage">({detectionCount})</span>
                  )}
                </span>
          ...
          <label className="flex cursor-pointer items-start gap-3 rounded-xl border border-clay/20 bg-clay/5 p-3 transition-colors hover:bg-clay/8">
            ...
            />
            <span>
              <span className="block text-sm font-semibold text-charcoal">Development cases</span>
              <span className="mt-0.5 block text-xs text-charcoal-muted">
                  ...
                  {layers.map((layer) => (
                    <li key={layer.id}>
                      <label className="flex cursor-pointer items-start gap-3 rounded-lg px-2 py-2 transition-colors hover:bg-sand/50">
                        ...
                        />
                        <span>
                          <span className="block text-sm font-medium text-charcoal">{layer.title}</span>
