...
async function buildFeatureContext(
  ...
  const searchGeometry = geometryEngine.geodesicBuffer(geometry as __esri.Polygon, 500, "meters");
  const query = async (layer: FeatureLayer | undefined, target: __esri.Geometry) => {
  ...
  const nearest = (features: __esri.Graphic[]) => {
    const distances = features
      ...
      .map((feature) => geometryEngine.distance(geometry, feature.geometry, "meters"))
  ...
  return {
    ...
    buildingFootprintM2: buildings.reduce((total, building) => {
      const intersection = building.geometry ? geometryEngine.intersect(geometry, building.geometry) : null;
...
export function KilimaniMap({
  ...
  useEffect(() => {
    ...
    const clickHandle = view.on("click", async (event: __esri.ViewClickEvent) => {
      ...
      const featureHit = response.results.find(
        (result: __esri.ViewHit) =>
          result.type === "graphic" &&
          Object.values(featureLayers).includes((result as __esri.GraphicHit).graphic.layer as FeatureLayer),
      ) as __esri.GraphicHit | undefined;
      ...
      if (featureHit?.graphic?.geometry) {
        ...
        const attributes = (featureHit.graphic.attributes ?? {}) as Record<string, unknown>;
        const kind: PlannerFeatureKind = layerId === "parcels-landuse"
          ? "parcel"
              ...
              : layerId === "roads"
                ...
                : layerId === "rivers"
                  ...
                  : layerId === "river-buffer"
                    ? "river-buffer"
                    : "boundary";
        const context = await buildFeatureContext(layerId, featureHit.graphic.geometry, featureLayers);
        const layerView = await view.whenLayerView(layer);
        clearHighlight();
        ...
        view.goTo({ target: featureHit.graphic, zoom: kind === "boundary" ? 15 : 18 }).catch(() => undefined);
        return;
      }
  ...
  useEffect(() => {
    ...
    const searchConfigs = MAP_LAYERS.filter((config) => config.searchFields?.length && (config.id === "parcels-landuse" || config.id === "buildings-parcel-join"));
    let cancelled = false;
    (async () => {
      for (const config of searchConfigs) {
        const layer = featureLayersRef.current[config.id];
        if (!layer) continue;
        const where = config.searchFields!.map((field) => `${field} LIKE '%${value}%'`).join(" OR ");
        try {
          const result = await layer.queryFeatures({ where, returnGeometry: true, outFields: ["*"] });
          ...
          const kind: PlannerFeatureKind = config.id === "parcels-landuse" ? "parcel" : "building";
          const context = await buildFeatureContext(config.id, graphic.geometry, featureLayersRef.current);
          onFeatureSelectRef.current?.({ kind, layerId: config.id, layerTitle: config.title, geometry: graphic.geometry, attributes, context });
          setSearchMessage(null);
          view.goTo({ target: graphic, zoom: kind === "parcel" ? 18 : 19 }).catch(() => undefined);
          return;
        } catch {
          // Continue to the next searchable layer when one service is unavailable.
        }
      }
      if (!cancelled) setSearchMessage("No parcel or building matched that value.");
    })();
  ...
  return (
    ...
    >
      ...
      {mapStatus === "ready" && (
        <>
          <form onSubmit={(event) => { event.preventDefault(); setSearchMessage(null); setSearchTerm(searchValue); }} className="pointer-events-auto absolute left-4 top-4 z-20 flex w-[min(22rem,calc(100%-2rem))] gap-2">
            <input value={searchValue} onChange={(event) => setSearchValue(event.target.value)} placeholder="Search parcel, LR number, or building ID" aria-label="Search parcel or building" className="min-w-0 flex-1 rounded-xl border border-sand bg-off-white/95 px-3 py-2 text-xs text-charcoal shadow-soft backdrop-blur-md" />
            <button type="submit" className="rounded-xl bg-forest px-3 py-2 text-xs font-semibold text-off-white shadow-soft hover:bg-forest-dark">Search</button>
          </form>
          {searchMessage && <p className="pointer-events-none absolute left-4 top-16 z-20 rounded-lg border border-clay/30 bg-off-white/95 px-3 py-2 text-xs font-semibold text-clay-dark shadow-soft">{searchMessage}</p>}
        <div className="pointer-events-auto absolute right-4 top-4 z-20 flex gap-2">
          <button type="button" onClick={() => viewRef.current?.goTo(KILIMANI_WARD_EXTENT).catch(() => undefined)} className="inline-flex items-center gap-1.5 rounded-xl border border-sand bg-off-white/95 px-3 py-2 text-xs font-semibold text-charcoal shadow-soft backdrop-blur-md hover:border-forest" aria-label="Zoom to Kilimani">
            <Home className="h-3.5 w-3.5 text-forest" /> Kilimani
          </button>
          {(selection || proximity) && <button type="button" onClick={clearSelection} className="inline-flex items-center gap-1.5 rounded-xl border border-sand bg-off-white/95 px-3 py-2 text-xs font-semibold text-charcoal shadow-soft backdrop-blur-md hover:border-clay" aria-label="Clear map selection"><X className="h-3.5 w-3.5" /> Clear</button>}
        </div>
        </>
      )}
      {mapStatus === "loading" && (
