...
export async function getCases(query: CaseQuery = {}): Promise<PaginatedResponse<DevelopmentCase>> {
...
export async function getCase(id: string): Promise<{ success: boolean; data: DevelopmentCase }> {
  return fetchJson(`/api/v1/cases/${encodeURIComponent(id)}`);
}

export async function getCasesGeoJSON(query: CaseQuery = {}): Promise<GeoJSON.FeatureCollection> {
  const params = new URLSearchParams();
  Object.entries(query).forEach(([k, v]) => {
    if (v != null && v !== '') params.set(k, String(v));
  });
...
export async function getCaseStats(): Promise<{ success: boolean; data: CaseStats }> {
...
export async function uploadEvidence(
  ...
  const response = await fetch(`${API_BASE}/api/v1/cases/${encodeURIComponent(id)}/evidence`, {
    method: 'POST',
    headers: authHeaders(),
    body: form,
  });
...
export async function getUnpromotedDetectionCount(minConfidence = 0.75): Promise<{
  success: boolean;
  data: { unpromoted: number; min_confidence: number };
}> {
  return fetchJson(`/api/v1/cases/unpromoted-detections?min_confidence=${minConfidence}`);
}
