...
export function usePropertyRecordQuery(id: string | undefined, enabled = true) {
  return useQuery({
    queryKey: caseKeys.propertyRecord(id ?? ""),
    queryFn: () => getPropertyRecord(id!).then((response) => response.data),
...
function normalizeStats(raw: Record<string, number | undefined>) {
  return {
    ...
    pendingVerification:
      raw.pending_verification ?? raw.pendingVerification ?? 0,
...
export function useNormalizedCaseStats() {
  ...
  return {
    ...q,
    stats: q.data
      ? normalizeStats(q.data as unknown as Record<string, number>)
      : undefined,
  };
}
...
export function useUpdateCaseStatus() {
  ...
  return useMutation({
    ...
    onMutate: async ({ id, status }) => {
      ...
      if (prev) {
        qc.setQueryData<DevelopmentCase>(caseKeys.detail(id), {
          ...prev,
          status,
        });
      }
