...
const { errorHandlerMiddleware, AppError } = require('./middleware/errorHandler');
...
const getEngineInfo = (req, res) => {
  res.json({
    ...
    description: 'Sentinel-2 Multi-Temporal Earth Observation Detection & Automated Model Training Engine for Kilimani Ward, Nairobi.',
    status: 'operational',
    ...
    pipeline: 'SENTINEL-2 -> TEMPORAL COMPOSITE -> SPECTRAL DIFFERENCING -> PRITHVI-EO ADAPTER -> ENSEMBLE -> FALSE-POSITIVE FILTER -> GEOJSON',
    endpoints: {
      health: '/health',
      models: {
